...
########################################################################################################################
#
# File: test_xrd_interface_parser_nc.py
...
class TestXRDInterfaceParserNC(unittest.TestCase):
    """Unit tests for XRD interface parser (NC)"""

    def setUp(self):
        """Set up test fixtures"""
        self.mock_device = MagicMock()

    @patch('jeypyats.parsers.xrd.xrd_interface_parser_nc.logger')
    ...
    @patch('jeypyats.parsers.xrd.xrd_interface_parser_nc.logger')
    def test_get_interface_status_not_ok(self, mock_logger):
        """Test interface status retrieval when reply is not ok"""
        mock_reply = MagicMock()
        mock_reply.ok = False
        mock_reply.xml = """<?xml version="1.0" encoding="UTF-8"?>
        <rpc-reply xmlns="urn:ietf:params:xml:ns:netconf:base:1.0" message-id="1">
